        ...
        <View className='flex-col items-start justify-center mt-5 px-5'>
            <Text className='text-white text-xl font-bold'>{movie ?. title}</Text>
              ...
              <View className='flex flex-row justify-between w-1/2'>
                  <MovieInfo label="Budget" value={ typeof movie?.budget === 'number' && movie?.budget > 0 ? `$${(movie.budget / 1_000_000).toLocaleString()} M`: 'N/A'}/>
                  <MovieInfo label="Revenue" value={ typeof movie?.revenue === 'number' && movie?.revenue > 0? `$${Math.round(movie.revenue / 1_000_000).toLocaleString()} M`: 'N/A'}/>
              </View>
